...
impl AppState {
    ...
    pub fn new(formbd_client: formbd::Client, spatial_index: SpatialIndex, config: Config) -> Self {
        Self {
            formbd_client,
            spatial_index,
            config,
...
pub async fn serve(state: AppState) -> Result<()> {
    ...
    info!("FormBD-Geo listening on {}", addr);
...
async fn reindex_handler(State(state): State<Arc<AppState>>) -> Result<Json<ReindexResponse>, StatusCode> {
    ...
    // Fetch documents from FormBD
    let documents = state
        .formbd_client
        .fetch_collection(&state.config.formbd.collection)
        .await
        .map_err(|e| {
            tracing::error!("Failed to fetch from FormBD: {}", e);
    ...
    let entries: Vec<SpatialEntry> = documents
        .iter()
        .filter_map(|doc| {
            formbd::Client::extract_location(doc, &state.config.formbd.location_field)
...
fn results_to_entries(results: &[SpatialQueryResult], formbd_base_url: &str) -> Vec<ResultEntry> {
    results
        .iter()
        .map(|r| ResultEntry {
            formbd_id: r.entry.formbd_id.clone(),
            location: LocationResponse {
                lat: r.entry.lat(),
                lon: r.entry.lon(),
            },
            distance_km: r.distance_km,
            provenance_url: format!("{}/documents/{}", formbd_base_url, r.entry.formbd_id),
        })
        .collect()
}
