...
impl SpatialIndex {
    ...
    pub fn insert(&self, entry: SpatialEntry) {
        ...
        let rtree_entry = GeomWithData::new(point, entry.formbd_id);
    ...
    pub fn bulk_insert(&self, entries: Vec<SpatialEntry>) {
        let rtree_entries: Vec<RTreeEntry> = entries
            .into_iter()
            .map(|e| GeomWithData::new([e.lon(), e.lat()], e.formbd_id))
            .collect();
...
mod tests {
    ...
    fn test_insert_and_query_bbox() {
        ...
        let ids: Vec<_> = results.iter().map(|r| &r.entry.formbd_id).collect();
    ...
    fn test_query_radius() {
        ...
        assert_eq!(results[0].entry.formbd_id, "doc_london");
