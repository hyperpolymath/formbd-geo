...
struct Args {
    ...
    #[arg(short, long, default_value = "formbd-geo.toml")]
    config: PathBuf,

    /// Override FormBD API URL
    #[arg(long)]
    formbd_url: Option<String>,
...
async fn main() -> Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "formbd_geo=info,tower_http=debug".into()),
        )
    ...
    // Apply CLI overrides
    if let Some(url) = args.formbd_url {
        config.formbd.api_url = url;
    }
    ...
    info!("FormBD API: {}", config.formbd.api_url);
    info!("Listening on {}:{}", config.server.host, config.server.port);

    // Create FormBD client
    let formbd_client = formbd::Client::new(&config.formbd.api_url)?;

    // Create spatial index
    let spatial_index = index::SpatialIndex::new(config.index.max_memory_mb);

    // Create application state
    let app_state = api::AppState::new(formbd_client, spatial_index, config.clone());
